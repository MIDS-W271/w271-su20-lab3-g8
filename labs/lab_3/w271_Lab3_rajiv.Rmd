...
xt <- table(d3$speed_lim, d3$year) %>% reshape2::melt() %>% rename(SpeedLimit=Var1, Year=Var2, PercentageStates=value)
...
genplot <- function(df, col, legend){
  ...
  df2.plot <- ggplot(df2, aes(fill=get(col), y=NumberOfStates, x=Year))+
...
minage.plot <- genplot(d_data, "minage", "Minimim\nDrinking\nAge")
...
# License laws
gdl.plot <- genplot(d_data, "gdl", "Grad. Drivers \nLicense Law")
...
```{r warning=FALSE}
# TODO: Changing the base produces different significance
...
Transformation is needed for variables `unem`, `perc14_24` and `vehiclesmilespc`

```{r warning=FALSE}
#TODO: Should we transform

# divide perc14_24 / 100 so its a percent
# divide unem / 100 so its a percent
# log transform vehicmilespc / 100 so its a percent


stargazer(lm(totfatrte~factor(year)+bac08+bac10+perse+sbprim+sbsecon+sl70plus+gdl+perc14_24+unem+vehicmilespc, data=d_data), type="text")
